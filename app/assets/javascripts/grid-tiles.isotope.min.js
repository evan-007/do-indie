function init_grid_tiles() {

    console.log('called init_grid_tiles');

    var $container = jQuery('.grid-tiles'),
        $container_sidebar = jQuery('.grid-tiles-sidebar'),
        $container_album_gallery = jQuery('.grid-tiles-album-gallery');

    var $containerProxy = $container.clone().empty().css({
        visibility: 'hidden'
    }),
        $containerProxy_sidebar = $container_sidebar.clone().empty().css({
            visibility: 'hidden'
        }),
        $containerProxy_album_gallery = $container_album_gallery.clone().empty().css({
            visibility: 'hidden'
        });
    ...
    if (typeof window.grid_tiles_initialized != 'undefined')
        $(window).off("resize");
    ...
    jQuery(window).resize(function() {
        ...
        $container_sidebar.imagesLoaded(function() {
            $(window).trigger('resize');
        });

        $container_album_gallery.imagesLoaded(function() {
            $(window).trigger('resize');
        });

    });

    window.grid_tiles_initialized = true;
}